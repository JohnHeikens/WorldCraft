...
void bodypart::CalculateTransform()
{
	...
	applied = combine(transforms);
}

void bodypart::Draw(GraphicsObject* graphics, mat4x4 transform, color* fill, const vec3 camera, const vec3 direction)
{
	...
	graphics->DrawTrianglesPlain(&cubevb0, &cubeibo, &c, camera, mat4x4::cross(view, scalecentre), direction);
	for (bodypart* b : childs)
	{
		b->Draw(graphics, view, fill, camera, direction);
	}
}
