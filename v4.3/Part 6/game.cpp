...
void Draw() {
	...
	if (mode == gamemode::survival) 
	{
		...
		{
			Image* cur;
			int hungerpart = player->foodlevel - i;
			if (hungerpart < 0) //draw empty heart
			{
				cur = emptyhunger;
			}
			else if (hungerpart < 1)//draw half a heart
			{
				cur = halfhunger;
			}
			else //draw a full heart
			...
			mat3x3 imgtransform = mat3x3::scale2d(2);
			int hungersize = 0x10;
			graphics->FillImage(graphics->width - (i / 2 + 1) * hungersize, graphics->height - hungersize, hungersize, hungersize, *cur);
		}
	}
	...
	int posx = (int)floor(player->position.x), posy = (int)floor(player->position.y);
	cfp elevation = HeightNoise->Evaluate2d(posx, posy);
	PowIntSimple(elevation, ElevationPower);
	cfp moisture = MoistureNoise->Evaluate2d(posx, posy);
	PowIntSimple(moisture, MoisturePower);
	biome b = GetBiomeClimate(elevation, moisture);
	std::wstring caption = L"FPS: " + std::to_wstring(fps) + L",\tbiome: " + biomename[b] + L",\tmoisture: " + std::to_wstring(moisture);
	//then caption
	...
	if (foodlevel > 17 || foodlevel == 0) 
	{
		...
		else
		{
			if (foodlevel == 20) {
				if (foodticktimer == 10)
				{
					newhealth += min(foodsaturationlevel / 6.0, 1);
				}
			}
		}
		//cap health at a maximum of 20
		if (newhealth > 20) 
		{
			newhealth = 20;
		}
		health = max(newhealth, 1);//minumum health: half a heart
	}
