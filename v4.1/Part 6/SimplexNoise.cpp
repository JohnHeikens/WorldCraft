...
static inline int32_t fastfloor(fp floatingpoint) {

	int32_t i = static_cast<int32_t>(floatingpoint);

	return (floatingpoint < i) ? (i - 1) : (i);
...
fp SimplexNoise::noise3(fp x, fp y, fp z) {
	...
	fp s = (x + y + z) * F3; // Very nice and simple skew factor for 3D
	...
	fp X0 = i - t; // Unskew the cell origin back to (x,y,z) space

	fp Y0 = j - t;

	fp Z0 = k - t;

	fp x0 = x - X0; // The x,y,z distances from the cell origin
	...
	fp x1 = x0 - i1 + G3; // Offsets for second corner in (x,y,z) coords

	fp y1 = y0 - j1 + G3;

	fp z1 = z0 - k1 + G3;

	fp x2 = x0 - i2 + 2.0f * G3; // Offsets for third corner in (x,y,z) coords

	fp y2 = y0 - j2 + 2.0f * G3;

	fp z2 = z0 - k2 + 2.0f * G3;

	fp x3 = x0 - 1.0f + 3.0f * G3; // Offsets for last corner in (x,y,z) coords

	fp y3 = y0 - 1.0f + 3.0f * G3;

	fp z3 = z0 - 1.0f + 3.0f * G3;
