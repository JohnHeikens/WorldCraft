...
pig::pig(vec3 pos)
{
	...
	head = new bodypart(&body, vec3(0, bodysize.y * 0.5 + headsize.z * 0.2, headsize.z * 0.2), headsize, headsize * .5, { rotation( zup::left) });
	...
	leg00 = new bodypart(&body, vec3(-legoffset.x , -legoffset.y - offy,legoffset.z),legsize,legjoint, { rotation(zup::left) });
	leg10 = new bodypart(&body, vec3(legoffset.x , -legoffset.y - offy, legoffset.z), legsize, legjoint, { rotation(zup::right) });
	leg01 = new bodypart(&body, vec3(-legoffset.x , legoffset.y - offy, legoffset.z), legsize, legjoint, { rotation(zup::left) });
	leg11 = new bodypart(&body, vec3(legoffset.x , legoffset.y - offy, legoffset.z), legsize, legjoint, { rotation(zup::right) });//covered
