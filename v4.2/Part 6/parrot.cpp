...
{//y+ = front
	...
	body.rotations[0].angle = math::PI * 0.2;//pitch
	...
	wing0 = DBG_NEW bodypart(&body, vec3(-sidewingoffset.x,sidewingoffset.y,sidewingoffset.z), sidewingsize, vec3(sidewingsize.x - sidewingjoint.x,sidewingjoint.y,sidewingjoint.z), { rotation(zup::left,math::PI * 0.1),rotation(zup::up) });
	wing1 = DBG_NEW bodypart(&body, vec3(sidewingoffset.x, sidewingoffset.y, sidewingoffset.z), sidewingsize, vec3(sidewingjoint.x, sidewingjoint.y, sidewingjoint.z), { rotation(zup::left,math::PI * 0.1),rotation(zup::up) });
	...
	head0 = DBG_NEW bodypart(&body, head0offset, head0size, head0joint, { rotation(zup::right,math::PI * -.2) });
